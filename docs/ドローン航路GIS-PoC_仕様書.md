...
版：0.7（Phase B複数レイヤ投入版）  
作成日：2026年8月6日  
ステータス：Phase A受入基準達成、Phase Bは建物・道路・土砂災害・洪水浸水を部分達成（地形・土地利用は未着手）  
...
2. 同データセットの建物、地形、洪水浸水、土砂災害、土地利用、道路の各レイヤを取得する。**実績**：建物・道路・土砂災害・洪水浸水（`udx/bldg`・`udx/tran`・`udx/lsld`・`udx/fld`配下のCityGML）を取得。ZIP全体（580MB）はダウンロードせず、HTTP RangeリクエストでZIP中央ディレクトリと対象ファイルだけを部分取得した（§14-3・§14-5）。地形（`udx/dem`は1ファイル300〜500MBのため対象外と判断）・土地利用（`udx/luse`は対象2次メッシュで約19MB、未着手）は未取得。
3. 建物と地形を `SpaceInfra-cpp` により変換できるか確認し、変換結果を地物ボクセルとして登録する。**実績：`SpaceInfra-cpp`は使用不可と判断（§10-3参照）**。Windows専用のVisual Studioプロジェクト（VS2022＋vcpkgの手動セットアップが必要、CMake等の自動ビルドは無し）であり、Renderでは動かせない。加えて、Laravel側にも地物ボクセルを登録するAPIが存在しない（`GroundFeatureVoxelController`等は取得専用、`UserApiController`の"voxel"系メソッドも取得・ダウンロード専用）ことが判明した。代替として、CityGMLをPython（`parse_bldg.py`・`parse_layer.py`）でフットプリント・空間ID（ズーム17）を抽出したJSONに変換し、fork側に追加したArtisanコマンド（`plateau:import-buildings`／`plateau:import-features`）で`ground_feature_objects`へ直接登録した。建物29件・道路30件・土砂災害13件・洪水浸水20件を登録。`object_cd`は1=建物・2=道路・3=土砂災害・4=洪水浸水という本PoC独自の割り当て（実コードに定義が無いため）。
4. 洪水浸水、土砂災害、土地利用、道路を、エリアまたは汎用オブジェクトとして登録する。**土地利用以外は実績あり**。ただし`area`／`general_purpose`経由ではなく、建物と同じ`ground_feature_objects`への直接登録とした（`area`経由だとPhase Aで判明したネイティブ変換処理依存の問題を再度踏むため）。土地利用（`udx/luse`）は未実施。
5. 航路周辺の地物・エリアをAPIで取得できることを確認する。**建物・土砂災害で実績あり**：`GET /ground_feature_voxel`で、既定航路の位置にある建物1件・土砂災害1件を取得できることを確認した（同APIは`->first()`で1件のみ返す設計のため、この件数が正しい）。道路・洪水浸水は登録済みだが、既定航路の空間IDと一致するデータが無いため未確認（**同APIが空間IDの完全一致検索のみで周辺検索に対応していないための制約であり、データや実装の不備ではない**）。地形・土地利用は未実施につき確認していない。
