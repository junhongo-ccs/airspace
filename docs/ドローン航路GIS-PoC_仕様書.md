...
版：1.3（DID地区の常時表示対応版）  
作成日：2026年8月7日  
ステータス：Phase A受入基準達成、Phase Bは建物・道路・土砂災害・洪水浸水・土地利用を部分達成（地形のみ未着手）、§5-3高度基準統一は建物レイヤ＋150m高度制限に限り達成、飛行禁止エリアはDID地区（実データ）を追加。Phase C（Viewer）はStreamlitからReact＋FastAPI BFFへ実装が移行済み（Streamlit版の正式廃止は未決定、§4-1参照）。React版はDID地区・建物のジオメトリを地図に描画でき、DID地区は航路登録前から常時表示する  

| 版 | 日付 | 変更内容 | 対応レビュー |
|---|---|---|---|
| 1.3 | 2026年8月7日 | React版のDID地区表示を、航路登録・照会の事後確認から、登録前の常時表示へ変更した。「危険区域を避けてルートを設計する」というGISの本来の使い方に合わせ、Laravelへの照会を伴わない`GET /known_prohibited_areas`（静的な参照ジオメトリのみ返す）を新設し、React版起動時に一度呼んで航路の登録有無に関わらず地図へ表示するようにした。航路登録後の`POST /query_prohibited_areas`（Laravelへの実際の登録確認・交差判定）とは役割を分離した。§7-3手順4を更新。 | 実装セッション（2026-08-07、[進捗ログ](進捗ログ.md)参照） |
...
4. 地図上に航路と空域レイヤを重ねる。**実績（航路・建物・DID地区）**：MapLibre GLで航路を
   ...
   （§7-2手順3参照）。**DID地区は航路登録前から常時地図に表示する**（同日追記）：
   「危険区域を避けてルートを設計する」というGISの本来の使い方に合わせ、Laravelへの
   照会を伴わない`GET /known_prohibited_areas`（静的な参照ジオメトリを返すのみ）を
   起動時に一度呼び、航路の登録有無に関わらず地図へ表示する。航路登録後の
   `POST /query_prohibited_areas`（Laravelへの実際の登録確認、交差判定）とは役割を
   分離した。
